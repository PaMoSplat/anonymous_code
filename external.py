...
def calc_lpips(img1, img2, net='alex', device='cuda'):
    global lpips_model
    if lpips_model is None:
        lpips_model = lpips.LPIPS(net=net).to("cuda")
    img1 = img1.unsqueeze(0)
    img2 = img2.unsqueeze(0)
    lpips_value = lpips_model(img1, img2)
    if lpips_value.shape[0] == 1:
...
def update_params(new_params, params):
    ...
    for k, v in new_params.items():
        updated_params[k] = torch.nn.Parameter(v.requires_grad_(True))
...
def remove_points_wo_var(to_remove, params, optimizer):
    to_keep = ~to_remove
    keys = [k for k in params.keys() if k not in ['cam_m', 'cam_c']]
    for k in keys:
        group = [g for g in optimizer.param_groups if g['name'] == k][0]
