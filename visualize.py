...
from helpers import setup_camera, quat_mult, params2rendervar
from external import build_rotation
from colormap import colormap
from copy import deepcopy

'''Note: You can change these to vis in different model'''

# RENDER_MODE = 'color'  
# RENDER_MODE = 'depth'  
# RENDER_MODE = 'centers'  
RENDER_MODE = 'partcolor'  

# ADDITIONAL_LINES = None  
ADDITIONAL_LINES = 'trajectories'  
# ADDITIONAL_LINES = 'rotations'  
...
def load_scene_data(seq, exp, seg_as_col=False):
    ...
    scene_data = []
    print("Total timestamp is: ", len(params['means3D']))
    for t in range(len(params['means3D'])):
        rendervar = {
            'means3D': params['means3D'][t],
            'colors_precomp': params['rgb_colors'][t] if not seg_as_col else params['part_colors'][t],
...
def render_group(w2c, k, timestep_data):
    with torch.no_grad():
        grouprendervar = params2rendervar(timestep_data)
        grouprendervar['colors_precomp'] = timestep_data['part_colors']
        cam = setup_camera(w, h, k, w2c, near, far)
        im, _, depth, = Renderer(raster_settings=cam)(**grouprendervar)
        return im, depth
...
def visualize(seq, exp):
    scene_data, is_fg = load_scene_data(seq, exp, seg_as_col=RENDER_MODE == 'partcolor')   
